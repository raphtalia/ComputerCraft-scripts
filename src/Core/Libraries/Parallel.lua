...
local function runUntilLimit(_routines, _limit, _timeout)
    ...
    local eventData = {n = 0}
    while true do
        if _timeout and runStart - os.clock() > _timeout then
            return nil
        end
        for n = 1, count do
            local r = _routines[n]
...
function Parallel.waitForAny(tFns, timeout)
    local routines = create(tFns)
    return runUntilLimit(routines, #routines - 1, timeout)
end
...
function Parallel.waitForAll(tFns, timeout)
    local routines = create(tFns)
    return runUntilLimit(routines, 0, timeout)
end
